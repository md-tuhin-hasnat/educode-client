...
export default function StudentNotificationsPage() {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (parsed.type === 'NOTIFICATION' && parsed.payload) {
          ...
          // Strictly ensure notification is either global (userId is null) OR matches this student
  ...
  const commentsCount = notifications.filter((n) => n.category === 'COMMENT' || n.category === 'REPLY' || n.category === 'POST').length;
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'POST':
        return { icon: faComments, color: 'text-cyan-400', label: 'Stream Post' };
      case 'EXAM_ALERT':
      ...
      <div className="bg-slate-900/60 border border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex items-center space-x-1 bg-slate-950 p-1 border border-slate-800 rounded-xl">
            ...
            {['ALL', 'SUBMISSION', 'POST', 'COMMENT', 'EXAM_ALERT'].map((cat) => (
              <button
                key={cat}
