...
export default function TestCaseRunnerPanel({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-[#141414] text-slate-200 border-t border-slate-800 select-text overflow-hidden">
      {/* Header Toolbar */}
      <div className="bg-slate-900/90 px-3.5 py-2 border-b border-slate-800 flex flex-col gap-2 shrink-0">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="flex items-center space-x-2.5 flex-wrap">
            ...
            <div className="flex items-center space-x-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-300 border border-amber-500/30">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {onAddCustomTestCase && (
            <button
              ...
              className="px-2.5 py-1 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-medium border border-slate-700 flex items-center space-x-1 transition-colors"
          ...
          {onRunCategory && sampleCount > 0 && (
            <button
              ...
              disabled={isRunning}
              className="px-2.5 py-1 rounded-md bg-emerald-600/80 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-semibold shadow-sm flex items-center space-x-1 transition-all border border-emerald-500/40"
          ...
          {onRunCategory && pretestCount > 0 && (
            <button
              ...
              disabled={isRunning}
              className="px-2.5 py-1 rounded-md bg-amber-600/80 hover:bg-amber-500 disabled:opacity-50 text-white text-xs font-semibold shadow-sm flex items-center space-x-1 transition-all border border-amber-500/40"
          ...
          <button
            ...
            className="px-3.5 py-1 rounded-md bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 hover:to-indigo-500 disabled:opacity-50 text-white text-xs font-bold shadow-md shadow-brand-600/30 flex items-center space-x-1.5 transition-all"
          >
            <FontAwesomeIcon
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden min-h-0">
        {/* Left Side: Real-time Live Log Box (conditional) */}
        {showLogs && (
          <LiveTestLogsView
            summary={summary}
            isRunning={isRunning}
        ...
        {/* Expandable Test Case Results List */}
        <div className="flex-1 flex flex-col bg-[#141414] overflow-hidden min-h-0">
          {/* Filter Bar */}
          <TestCaseFilterBar
            filter={filter}
            setFilter={setFilter}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 min-h-0">
            {testCases.length === 0 ? (
              ...
              testCases.map((tc, idx) => {
                ...
                const isHidden = isPretest || isSystem || !!tc.isHidden;
                return (
                  <div
                    key={tc.id || idx}
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 mt-0.5">
                          {isHidden ? '🔒 Hidden test case (runnable)' : tc.inputData ? `Input: ${tc.inputData.slice(0, 30)}...` : 'Standard input'}
                        </p>
                      </div>
                    </div>
